...
- `award_page` — Per-recipient printable award certificate (guilloche SVG border, Playfair Display/Lato fonts, navy/gold). **Two pages per recipient (PR #160)**: page 1 is the certificate front showing the award list with NO dollar amounts and no total; page 2 is a compact `column-count:2` "money insert" (name + awards + amounts + total, ~8.5pt, top third) that lands on the back under duplex printing so the cash folds inside and never shows on the front. Prize text is rendered through the `prize_label()` / `prize_detail()` Jinja macros — shared by the front list and the money insert so they can't drift; edit the macro, not two copies. Each recipient emits exactly two `page-break-after` pages — keep that count even if adding sections, or duplex front/back pairing breaks. **Print pagination (PR #161)**: `.certificate-wrap` is `display:flex` for screen centering, but `break-after`/`page-break-after` are ignored on flex items, so `@media print` forces `.certificate-wrap` to `display:block` and adds `break-inside:avoid` on `.certificate`/`.money-page`. Without the block override the print engine falls back to natural flow and certificates drift cumulatively / split mid-page (looks fine on screen, only breaks in Preview/printer). Do not remove the print `display:block` — same flex-defeats-pagination class as the payout-summary `thead` fix.
